...
class GitRepo:
    ...
    def status(self):
        ...
        rv = self.gitcmd(
            ...
            {"echo": False, "collect": True, "oprint": True},
        )
        if rv.rejected:
    ...
    def add(self, sa):
        ...
        rv1 = self.gitcmd(
            ["add"] + sa, {"echo": False, "collect": False, "oprint": True}
        )
        if rv1.rejected:
            utils.log(rv1.outs)
            fc = 0
            for f in sa:
                rv2 = self.gitcmd(
                    ["add", f], {"echo": False, "collect": False, "oprint": True}
                )
                if rv2.rejected:
                    utils.log(rv2.outs)
                else:
    ...
    def commit(self):
        ...
        rv = self.gitcmd(
            ["commit", "-a", "-m", "'abcde'"],
            {"echo": False, "collect": False, "oprint": True},
        )
        if rv.rejected:
    ...
    def push(self):
        ...
        rv = self.gitcmd(
            ["push", "origin", "master", "--porcelain", "--force"],
            {"echo": False, "collect": False, "oprint": True},
        )
        if rv.rejected:
...
if __name__ == "__main__":
    ...
    def getRepos():
        ...
        def run(prj):
            return GitRepo(wt=utils.prjDir(prj), gd=utils.gitDir(prj))
