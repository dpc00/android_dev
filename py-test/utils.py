...
prjList = ["py-test", "tools", "scala-test", "DirSyncApp"]
...
def shortcut(fn, target, params=None, wdir=None, icon=None, iindex=None, desc=None):
    ...
    if not desc is None:
        a += ["/D:" + desc]
    Dos4({"cmd": "cmd.exe", "args": a, "echo": True, "oprint": True, "shell": True})
...
def lnk2():
    ...
    shortcut(sfn, tfn, icon=ifn, iindex=0, wdir=wdir, desc="Eclipse Neon")
...
def lnk3():
    ...
    shortcut(sfn, tfn, icon=ifn, iindex=0, wdir=wdir, desc="Py-Test Repo")
...
def lnk4():
    ...
    # params = ""
    wdir = "%FLASH0%\\Projects"
    shortcut(sfn, tfn, icon=ifn, iindex=0, wdir=wdir, desc="jEdit")
...
def lnk5():
    ...
    # params = ""
    wdir = "%FLASH0%\\Projects"
    shortcut(sfn, tfn, icon=ifn, iindex=0, wdir=wdir, desc="PyCharm")
...
def lnk6():
    ...
    # params = ""
    wdir = "%FLASH0%\\Projects"
    shortcut(sfn, tfn, icon=ifn, iindex=0, wdir=wdir, desc="Sublime Text 3")
...
def lnk7():
    ...
    # params = ""
    wdir = "%FLASH0%\\"
    shortcut(sfn, tfn, icon=ifn, iindex=0, wdir=wdir, desc="Comodo Cleaner")
...
def lnk9():
    ...
    # params = ""
    wdir = "%FLASH0%\\"
    shortcut(sfn, tfn, icon=ifn, iindex=0, wdir=wdir, desc="Process Explorer")
...
def lnk10():
    ...
    # params = ""
    wdir = "%FLASH0%\\"
    shortcut(sfn, tfn, icon=ifn, iindex=0, wdir=wdir, desc="WinDirStat")
...
def lnk11():
    ...
    # params = ""
    wdir = "%FLASH0%\\Projects\\py-test"
    shortcut(sfn, tfn, icon=ifn, iindex=0, wdir=wdir, desc="Python")
...
def getSTPackage(pf="Package Control.sublime-package"):
    ...
    ipp = expES(
        r"%FLASH0%\PortableApps\SublimeText3Portable\App\SublimeText3\Data\Installed Packages"
    )
    log(ipp)
    ...
    try:
        by = urllib.request.urlopen(
            "http://packagecontrol.io/" + pf.replace(" ", "%20")
        ).read()
