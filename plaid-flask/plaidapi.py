...
class AccountInfo:
    def __init__(self, data):
        ...
        self.ts_consent_expiration = parse_optional_iso8601_timestamp(
            data["item"]["consent_expiration_time"]
        )
        self.ts_last_failed_update = parse_optional_iso8601_timestamp(
            data["status"]["transactions"]["last_failed_update"]
        )
        self.ts_last_successful_update = parse_optional_iso8601_timestamp(
            data["status"]["transactions"]["last_successful_update"]
        )
...
class PlaidAPI:
    def __init__(
        ...
        self.client = plaid.Client(client_id, secret, environment, suppress_warnings)
    ...
    def get_transactions(
        ...
        while True:
            ...
            ret += [Transaction(t) for t in response["transactions"]]

            if status_callback:
                status_callback(len(ret), total_transactions)
            if len(ret) >= total_transactions:
                break
